...
def generate_er_random_bipartite_graph(m, n, p):
    ERnet = nx.Graph()
    left_nodes = range(0, m)
    right_nodes = range(m, m+n)
    
    ERnet.add_nodes_from(left_nodes, bipartite=0)
    ERnet.add_nodes_from(right_nodes, bipartite=1)
    
    nei = math.ceil(m * p)
    for right_node in right_nodes:
        select_edges = np.random.choice(left_nodes, nei, replace=False)
        for left_node in select_edges:
            
            ERnet.add_edge(left_node, right_node)
    
    return ERnet


def generate_ba_random_bipartite_graph(m, n):
    BAnet = nx.Graph()
    left_nodes = range(0, m)
    # 添加初始节点集合到二部图的左侧节点集合中
    BAnet.add_nodes_from(left_nodes, bipartite=0)  # 左侧节点
    right_nodes = range(m, m+n)
    BAnet.add_nodes_from(right_nodes, bipartite=1)
    # 幂律分布参数
    exponent = 0.05
    size = 1000

    # 生成幂律分布数据
    data = np.random.power(exponent, size)

    # 选择一些点
    selected_points = np.random.choice(data, size=n, replace=False)

    # 迭代添加新节点到二部图的右侧节点集合中，并与左侧节点建立边连接
    for j in range(n):
        new_node = right_nodes[j]
        BAnet.add_node(new_node, bipartite=1)  # 右侧节点

        left_nodes_num = math.ceil(m*data[j])  # 从幂律分布中随机选择左侧节点
        selected_left_nodes = np.random.choice(left_nodes, left_nodes_num, replace=False)
        for left_node in selected_left_nodes:
            BAnet.add_edge(new_node, left_node)

    return BAnet


def draw_degree(bipartite_graph):

    edges = [node for node in bipartite_graph.nodes if bipartite_graph.nodes[node]['bipartite'] == 0]
        
    nodes = [node for node in bipartite_graph.nodes if bipartite_graph.nodes[node]['bipartite'] == 1]
    e_distribution = {}
    v_distribution = {}
    for edge in edges:
        degree = len(bipartite_graph[edge])
        if degree not in e_distribution.keys():
            e_distribution[degree] = 1
        else:
            e_distribution[degree] += 1
                
    for node in nodes:
        degree = len(bipartite_graph[node])
        if degree not in v_distribution:
            v_distribution[degree] = 1
        else:
            v_distribution[degree] += 1
    ...
    max_e_degree = max(e_distribution.keys())

    for key in range(max_e_degree+1):
        e_x_val.append(key)
        if key in e_distribution.keys():
            value = e_distribution[key] 
            e_y_val.append(value)
        else:
            e_y_val.append(0)
    ...
    max_v_degree = max(v_distribution.keys())

    for key in range(max_v_degree+1):
        v_x_val.append(key)
        if key in v_distribution.keys():
            value = v_distribution[key] 
            v_y_val.append(value)
        else:
            v_y_val.append(0)

    
    ##画图
    plt.title('result of degree distribution of edges and nodes ')
    plt.loglog(e_x_val, e_y_val, color = 'blue', marker = 'o',label='edges', linewidth = 2)
    plt.loglog(v_x_val, v_y_val, color = 'red', marker = '+',label='nodes', linewidth = 2)
    plt.legend() # 显示图例
    plt.show()
    return


def function_u(graph):
    # 初始化节点状态
    ...
    score_u = {}
    for node in Ini_V:
        degree = len(E_list[node])
        
        node_nei = Adj[node]
        node_edge = E_list[node]
        tmp = 0
        for nei in node_nei:
            #print(E_list[nei])
            codegree_node_nei = len(set(node_edge).intersection(set(E_list[nei])))
            tmp += codegree_node_nei
            if codegree_node_nei >=2 :
                degree += codegree_node_nei
        if tmp != 0 :
            score_u[node] = degree/tmp
        else:
            score_u[node] = 0
    return score_u


def simulate_sir_model(graph, infected_nodes, infection_rate, recovery_rate, num_iterations, str_graph, index):
    res = []
    # 统计评价指标
    ...
    for iteration in range(num_iterations):
        ...
        infected_num[iteration] = infected_count/nodes_num

        recovered_count = sum(node_state == 'R' for node_state in node_states.values())
        recovered_num[iteration] = recovered_count / nodes_num  
        # 更新最大感染人数
    ...
    if index == 0:
        # 绘制感染节点变化图
        plt.plot(list(infected_num.keys()), list(infected_num.values()), color = 'blue', marker = 'o', label='S', linewidth = 2)
        plt.plot(list(recovered_num.keys()), list(recovered_num.values()),color = 'red', marker = '+', label='R', linewidth = 2)
        plt.xlabel('Iteration')
        plt.ylabel('Infected Nodes')
        plt.title('Number of Infected Nodes and Recovered nodes over Iterations')
        plt.legend() # 显示图例
        plt.show()
        # 打印评估结果
    ...
    res.append(max_keys)
    res.append(infected_num[max_keys])
    res.append(infected_duration)
    res.append(recovered_num[infected_duration])
...
#Graph = generate_er_random_bipartite_graph(m_edges, n_nodes, p)
Graph = generate_ba_random_bipartite_graph(m_edges, n_nodes)
draw_degree(Graph)
...
def SIR_model_node(G_list):
    graph_str = ['Ini_G', 'Gen_G']
    for i in range(len(G_list)):
        final_node_states = simulate_sir_model(G_list[i], infected_nodes, infection_rate, recovery_rate, num_iterations, graph_str[i],index = 0)
    return

SIR_model_node([Ini_Graph,Gen_G])


#全局节点
def SIR_model_graph(G_list):
    graph_str = ['Ini_G', 'Gen_G']
    for i in range(len(G_list)):
        graph = G_list[i]
        Score_graph = function_u(graph)
        node_sir = {}
        for node in Score_graph.keys():
            infected_nodes = list([node])
            final_node_states = simulate_sir_model(graph, infected_nodes, infection_rate, recovery_rate, num_iterations, graph_str[i], index = 1)
            node_score = Score_graph[node]
            node_sir[node_score] = final_node_states
        ...
        stable_step = []
        recovered_rate = []
        for key, value in node_sir.items():
            x_val.append(key)
            infected_step.append(value[0])
            infected_rat.append(value[1])
            stable_step.append(value[2])
            recovered_rate.append(value[3])

        plt.plot(x_val, infected_rat,color = 'red', marker = '+', label='infected_rat', linewidth = 2)
        plt.plot(x_val, recovered_rate, color='skyblue', marker = '*',label='recovered_rate', linewidth = 2)
        ...
        plt.plot(x_val, infected_step, color = 'blue', marker = 'o', label='infected_step', linewidth = 2)
        plt.plot(x_val, stable_step, color='green', marker = '+',label='stable_step', linewidth = 2)
        plt.xlabel('node score')
        plt.ylabel('step')
        plt.title('Number of Infected step and Recovered step over Iterations')
        plt.legend() # 显示图例
        plt.show()
...
#全局
SIR_model_graph([Ini_Graph,Gen_G])
